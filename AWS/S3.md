...
## S3 Bucket Policy
Line `Policy`, to configure access to specified resouce in S3. for e.x: only allow put object name `index.html`
...
![Server Logging](./assets/s3/s3_log.png)
...
![Hosting static website](./assets/s3/s3_static_web.png)
...
![Transfer Acceleration](./assets/s3/s3_transfer_accleration.png)
...
- Read after write eventually consitency when POST
- Read eventually when PUT and DELTE

### Trick & Tip

- For client to upload image, you can use `PostSignedUrl` to generate upload URL (has defaul timeout to 15min)
- To cache object in S3 (in case CloudFront is acts as CDN to S3), you can add `Cache-Control` or `Expired` in object's metadata by the following step
	1.  Choose **Actions**, then choose **Edit metadata**.

	2.  In the **Add metadata** panel, do the following:
